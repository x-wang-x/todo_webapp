...
import { setData } from "../module/storeData";
interface props {
  id: string;
...
const add = (title : string,group_id: string,content: string,id : number) => {
  setData('index',{
    group_id : group_id,
    content_id : id,
    title : title
...
const getAdd = () => {
  ...
  const content = document.getElementById('txt') as HTMLTextAreaElement
  const group_idx : HTMLSelectElement = document.getElementById('sbox') as HTMLSelectElement
  
  add(title.value,group_idx.value,content.value,5) 
}
      ...
      >
        <div className="flex flex-col p-4 w-full h-full">
          ...
          <div className="grow flex flex-col">
            ...
            <select id="sbox" className="bg-wx-dark border border-wx-light py-1 mb-3 pl-2">
              <option value={1}>Group 1</option>
              <option value={2}>Group 2</option>
            </select>
            <span className="mb-1">Write Your Notes : </span>
            <textarea id="txt" className="h-full resize-none overflow-scroll mb-3 pl-2 border border-wx-light bg-wx-dark text-wx-light focus:outline-none"></textarea>
          </div>
          {/* <!-- Modal footer --> */}
          <div className="flex-none mt-3 pt-1 flex items-end border-t-2 border-wx-light justify-end gap-2">
            <Button onClick={() => getAdd()} text="Add" />
